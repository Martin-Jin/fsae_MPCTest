#!/bin/bash
# Language: bash
# Title: One-shot launcher for the step-input yaw-cap identification test
#
# NOTE: the shebang MUST stay on line 1. It previously sat on line 3 below two
# header comments, which meant the kernel never saw it: the script then ran
# under whichever shell invoked it, and under dash it died immediately on
# `set -o pipefail`.
#
...
#   4. runs the steering_step sweep in the foreground
...
#
# Why this exists: the sweep (run_steering_sysid.sh) established THAT FSDS
# caps yaw rate above ~6 m/s. This identifies WHICH mechanism does it --
# a hard yaw limit, a speed-scaled steering authority, or active damping --
# from the transient after a step steering input. See
#   fsae_MPCTest/docs/planning_control_sync.md
#   -> "RESULT: FSDS caps yaw rate above ~6 m/s"
#
# Usage:
#   ./run_steering_step.sh                       # defaults (12 trials, ~3 min)
#   ./run_steering_step.sh --quick               # 2 trials, fastest check
#   ./run_steering_step.sh --no-sim              # FSDS already running
#   ./run_steering_step.sh -p home_radius:=25.0  # pass through to the node
...
for a in "$@"; do
    case "$a" in
        ...
        --quick)  EXTRA_ARGS+=(-p 'speeds:=[4.0, 10.0]'
                               -p 'steer_cmds:=[1.0]') ;;
        *)        EXTRA_ARGS+=("$a") ;;
    esac
done
...
cleanup() {
    ...
    pkill -f 'steering_step'  2>/dev/null
...
echo "============================================="
echo "🔬 Step-input test (yaw-cap mechanism ID)"
echo "============================================="
...
echo "  The car drives straight, then steps the steering and holds."
echo "  Ctrl+C aborts cleanly; the partial log stays valid."
echo "============================================="
echo ""

BEFORE=$(ls -1 "$LOG_DIR"/steering_step_*.csv 2>/dev/null | wc -l)
...
bash -c '
    ...
    source install/local_setup.bash && \
    shift && \
    ros2 run fsae_control steering_step --ros-args "$@"
' _ "$HOST_ROS2_DIR" "${EXTRA_ARGS[@]}"

# --- 5. Analyse -------------------------------------------------------------
NEWEST=$(ls -t "$LOG_DIR"/steering_step_*.csv 2>/dev/null | head -1)
AFTER=$(ls -1 "$LOG_DIR"/steering_step_*.csv 2>/dev/null | wc -l)
echo ""
echo "============================================="
if [ -n "$NEWEST" ] && [ "$AFTER" -gt "$BEFORE" ]; then
    ...
    ( cd "$HOST_REPO_ROOT/fsae_MPCTest" && \
      python3 -m tuner.steering_step_analysis "$NEWEST" )
    echo ""
