...
class MPCParams:
    ...
    nmpc_curv_rate_ref_enabled: bool = field(default=False, metadata={"unit": "bool", "desc": "EXPERIMENTAL: retarget the steering-RATE cost from zero onto the rate the reference path's own curvature change justifies (nmpc_core._du_ref), so a constant-radius corner still penalises every wiggle while a corner entry/S-bend is not penalised for the steering change it genuinely needs. Offline-measured as an order of magnitude too small to explain the chatter it targets (~0.44 vs ~4.8 deg/step) and offline A/Bs came back flat-to-slightly-worse -- kept for live evaluation only. CAUTION: dkappa/ds amplifies curvature noise, so a live planner centreline (vs a static raceline) may make this counterproductive. Default False, unvalidated", "controller": "nmpc_only"})
    nmpc_curv_rate_ref_gain: float = field(default=1.0, metadata={"unit": "unitless", "desc": "scales the curvature-justified steering-rate target; 1.0 = the full small-angle kinematic prediction, lower = less benefit-of-the-doubt given to the path. NOT a -1-inherit field (there is no LTV-QP counterpart to inherit from). Only read when nmpc_curv_rate_ref_enabled is True", "controller": "nmpc_only"})
