...
it and fold anything genuinely load-bearing from it into this file instead.
...
- **`fsae_autonomous`** is the **production repo** — what actually gets put
  ...
  because it only ever receives a deliberate, tested push (see below), not
  a continuous mirror.
...
- **Git layout** ("four independent repos" below) — four repos, one
  read-only, one (`fsae_autonomous`) pushed to only rarely and only to a
  branch. Silently breaks: an agent commit landing in `fsae_planning` when
  it should have stayed local, or an unasked push landing in
  `fsae_autonomous`; `git status` at the wrong root not showing the other
  repos' changes at all.
...
- **`fsae_autonomous`** — the production repo, and pushes here are
  deliberately rare and gated: **we only push to a branch** (never
  `main` directly for this repo), and **only** once a major version
  upgrade is fully tested and working. **Do not push to `fsae_autonomous`
  unless explicitly asked**, even if a change offline-validates cleanly —
  offline validation is not the bar for this repo, a full tested car-ready
  upgrade is. **Pull `main` before working in this repo each session** —
  it moves over time independently of the other three (other contributors,
  or your own past sessions' pushed branches merged upstream), so a local
  checkout can go stale between sessions in a way that isn't true of the
  same-session-owned tuning repos.
...
If a failing run looks flaky rather than genuinely broken, re-run it at
...
- The correctness bar for the area touched has actually been run (not just
...
- No new files added to `fsds_simulator/` that weren't already there (see
  "Third copy" above), and nothing committed/pushed to `fsae_planning`.
