...
| Speed-target deficit cap | `sim/rollout_core.py` (`SPEED_TARGET_DEFICIT_MAX`) | `fsds_simulator/.../mpc/mpc_controller.py` (same name) | `5.0` m/s (was 2.5), module constant on both sides, no ROS param/launch-arg override, so changing it means editing THREE files. Raised from 2.5 after measuring it as the binding constraint on acceleration for 36.8% of a lap, not the launch guard it was written as: see `docs/logs/nmpc_progress_term_investigation.md`. Offline-validated only, NOT yet live-tested |
