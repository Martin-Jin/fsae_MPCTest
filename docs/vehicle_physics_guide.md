...
|---|---|---|---|
| `lf`, `lr` | Distance from the car's centre of mass to the front axle (`lf`) and rear axle (`lr`). | Moving `lf` up (mass further from front axle) puts more weight over the rear — car becomes more prone to understeer (front loses grip first). | Less weight over that axle — that end grips relatively better, other end worse. |
| `m` | Total mass of car + driver. | Everything gets harder: more force needed to accelerate, brake, and corner at the same rate. Tyres are more loaded (which can help peak grip a little, but hurts accel/braking much more). | Car responds faster to the same forces — quicker acceleration, braking, and direction changes. |
...
actually starts to *fall off* (this is what a slide or a spin feels like).
...
closely reproduce the S-shaped force curve real tyres actually produce on a
test rig.
...
| `Sh` (horizontal offset) | Shifts *where* zero net force actually occurs, along the slip-angle axis. | Shifts the "neutral point" of the tyre's response to one side. | Shifts it the other way, or removes the shift entirely at `Sh = 0`. |
...
| `Cl_pitch_sens` | How much braking (nose dipping down, "pitching forward") temporarily shifts downforce toward the front and away from the rear. | Braking creates a bigger front-grip boost / rear-grip reduction — can make the rear feel looser specifically while braking hard. | Braking has little effect on the front/rear downforce balance. |
