...
tracking purposes, and the live repo is out of scope for edits/commits from here;
see `fsae_planning/CHANGES.md` itself for full detail. Local `main` is
up to date with `origin/main` (no upstream drift), and everything below is
working-tree-only.
...
- `car_position` → `PoseStamped` (adds timestamp), needed for delay
  compensation.
- `look_radius` 18m → 25m, to keep ahead of the planner's own extended
  lookahead.

## Planning

- `_WALL_PLAN_HORIZON`/`look_radius` 15/18m → 25m, to match new
  braking-distance scan range.
- Path-seed rejection loosened (reject only clearly-behind points). The old
...
- `curvature_speed()` rewritten to propagate a real braking-distance limit
  per corner. The old version could demand more deceleration than the car can
  produce.
- Curvature scan denoised (resample + moving average). Planner refit noise
  was causing speed oscillation on straights.
- New `tracking_error_speed_gate()`, which slows down when badly off-path, since
  curvature alone can't detect that.
- New CSV-backed speed/path lookup (`map_path`/`path_map_path`), to bypass live
  re-derivation for already-mapped tracks.

## Control — MPC core

- Pose-age delay compensation (`predict_ahead()`, `_update_n_delay()`), to plan
  against predicted current state, not a stale measurement.
- `e_yd` now includes body-frame lateral velocity. The old formula silently
  dropped sideslip contribution.
- `e_y` now signed perpendicular projection, not nearest-point distance. The old
  version was only correct when the nearest point happened to be abeam.
...
- Fixed logged steering angle being computed from the wrong input scale
  (~2.3x inflated), which had been masking the rate-limit issue in prior test logs.
...
- **Removed the entire forward-scanning lookahead gain-scheduling family**
  ...
  precomputed `CornerMap` fast path), since reweighting today's cost based on a
...
- New `mpc_params.py`: pulls every `mpc_core.py` weight/gain/flag (Q/R/R_rate
  ...
  corner-factor rewrite, down from an original ~56, since that rewrite deleted
...
- Both nodes now slice tracked path from car's nearest point before
  curvature, fixing inconsistent curvature measurement between the two
  nodes.

## New files

- `scoring.py`, a verbatim copy of offline composite score, so live runs are
  graded identically to tuner rollouts.
- `cone_recorder.py`/`.launch.py`, which records a completed lap's cone map for
  offline reuse.
...
- `control/fsae_control/test/nmpc_offline_check.py`, which reproduces the NMPC
...
- `setup.py` (all four packages): `zip_safe=True` → `False`, which works around a
