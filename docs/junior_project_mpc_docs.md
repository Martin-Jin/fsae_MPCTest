...
**This was fixed on 2026-08-12** (`curvature_forcing_enabled` / `CURVATURE_FORCING_ENABLED`) by injecting the precomputed path's own curvature into the dynamics as a forcing term over the horizon: $-v_x\cdot\kappa(s_k)\cdot dt$ is added to the predicted $e_\psi$ at each step $k$, using the path's actual curvature at the arc-length position the car is predicted to reach by step $k$ (walked forward from the car's current position by $v_x\cdot k\cdot dt$ per step — see `_curvature_horizon_profile` in `mpc_core.py`). That gives the model a term that predicts heading/lateral error growing on its own as a modelled straight-line car approaches a real bend — the thing a plain reweighting of $Q$/$R$ can never do. It was verified with a synthetic constant-curvature path before being trusted: with the term off, commanded steering is exactly $0.000°$ 24 m before a 20 m-radius bend with zero tracking error; with it on, the car correctly leans toward the bend before any error exists at all, in both left and right directions.

A related, less obvious fix landed the same day: `steer_rate_anti_hunt` (a mechanism that heavily damps small steering-rate changes when the car looks "centred and straight") was tuned before this curvature term existed, so it read the state curvature-forcing deliberately produces on approach — small error, small early corrective steering — as noise to suppress, and was cancelling the new mechanism's whole effect. It now also relaxes once a corner is detected in the lookahead window, not just once the car is already turning.
