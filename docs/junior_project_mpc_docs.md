...
The standard fix (not yet implemented here) is to inject the precomputed path's own curvature into the dynamics as a forcing term over the horizon — e.g. adding $-v_x\cdot\kappa(s_k)\cdot dt$ to the predicted $e_\psi$ at each step $k$, using the path's actual curvature at the arc-length position the car is predicted to reach by step $k$. That gives the model a term that predicts heading/lateral error growing on its own as a modelled straight-line car approaches a real bend — the thing a plain reweighting of $Q$/$R$ can never do.
