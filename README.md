...
| `tuner/plot_playback.py` | Time-scrubbing map/telemetry viewer for a single run — signals + slider on the left, full trajectory and live planner-path overlay top right, zoomed current-section view (with live e_y/e_psi) bottom right. See [docs/developer_guide.md#scrubbing-a-single-runs-map-position-over-time](docs/developer_guide.md#scrubbing-a-single-runs-map-position-over-time). |
