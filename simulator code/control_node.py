# Title: control_node.py - ROS 2 MPC Control Loop
import math
import numpy as np
import rclpy
...
CONE_BRAKE_DIST  = 1.5   # Metres forward — slightly pulled back
CONE_BRAKE_WIDTH = 0.18  # Reduce to 0.18 (36cm total width) — prevents false cornering flags
...
class ControlNode(Node):
    def __init__(self):
        ...
        # Initialize the state-space LTV-MPC (Running at 20Hz matching the 0.05s timer)
        self._mpc = MPCController(dt=0.05, N=20)

        # Execution Timer (50ms control loop)
        self.create_timer(0.05, self._control_loop)
        self.get_logger().info('MPC Control node initialized and ready.')
    ...
    def _go_cb(self, msg: GoSignal) -> None:
        if not self._go_received:
            ...
            self.get_logger().info('GO signal intercepted. Launching control loop!')
    ...
    def _odom_cb(self, msg: Odometry) -> None:
        ...
        # Convert quaternion to Euler Yaw angle (Z-axis rotation)
    ...
    # ------------------------------------------------------------------
    # Core MPC Processing Loop
    # ------------------------------------------------------------------

    def _control_loop(self) -> None:
        cmd = ControlCommand()

        # Phase 1: Wait for track entry approval signal
        if not self._go_received:
            cmd.throttle = 0.0
            cmd.steering = 0.0
            cmd.brake    = 1.0  # Keep brakes locked at start line
            self.pub_cmd.publish(cmd)
            self.get_logger().info('Waiting for GO signal...', throttle_duration_sec=2.0)
            return

        # Phase 2: Safeguard against missing or dropped localization/planning packets
        path_stale = (
        ...
        if path_stale:
            ...
            cmd.steering = 0.0
            cmd.brake    = 1.0
            self._mpc.reset()  # Break solver continuity on sudden hard stops
            self.pub_cmd.publish(cmd)
            self.get_logger().warn('Trajectory path lost or stale! Emergency braking initiated.', throttle_duration_sec=1.0)
            return

        # Phase 3: Execute optimal control calculations via MPC solver
        ...
        # Phase 4: Emergency Cone Proximity Hazard Overrides
        ...
        if cone_parts:
            ...
            # Map global coordinates into local car relative frame
            x_car =  rel[:, 0] * cos_y + rel[:, 1] * sin_y
            y_car = -rel[:, 0] * sin_y + rel[:, 1] * cos_y
            
            # Scan inside the geometric braking corridor width constraints
        ...
        if too_close:
            cmd.throttle = 0.0
            cmd.brake    = 1.0
            self.get_logger().warn('Braking zone violated by track feature! Force overriding commands.', throttle_duration_sec=0.5)

        # Phase 5: Frame timestamp allocation and packet delivery
        cmd.header.stamp = self.get_clock().now().to_msg()
        self.get_logger().info(
            f'MPC_CMD thr={cmd.throttle:.2f} brk={cmd.brake:.2f} steer={cmd.steering:.3f} | '
            f'Vel={self._car_speed:.1f}/{self._desired_speed:.1f} m/s',
